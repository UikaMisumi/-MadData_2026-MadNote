...
async def get_papers(
    ...
    if category and category != "All":
        # Match category column or tags containing the value
        cat_match = df["category"] == category
        if "tags" in df.columns:
            tags_match = df["tags"].apply(
                lambda t: category in (t if isinstance(t, list) else [])
            )
            df = df[cat_match | tags_match]
        else:
            df = df[cat_match]
